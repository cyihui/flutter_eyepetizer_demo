...
class TopicDetailWidgetItem extends StatelessWidget {
  ...
  const TopicDetailWidgetItem({Key? key, required this.model})
      : super(key: key);
  ...
  Widget _headWidget() {
    var headerIcon = model.data?.header?.icon ?? '';
    return Row(
      children: <Widget>[
        Padding(
      ...
      children: <Widget>[
        Row(
          children: <Widget>[
            ...
            Padding(
              ...
              child: Text(
                  '${model.data?.content?.data?.consumption?.collectionCount ?? 0}',
        ...
        Row(
          children: <Widget>[
            ...
            Padding(
              padding: const EdgeInsets.only(left: 10),
              child: Text('${model.data?.content?.data?.consumption?.replyCount ?? 0}',
                  style: TextStyle(fontSize: 12, color: hitTextColor)),
            )
          ],
        ),
  ...
  List<Widget> _getTagWidgetList(TopicDetailItemData itemData) {
    var tags = itemData.data?.content?.data?.tags ?? [];
